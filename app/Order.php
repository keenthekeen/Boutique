...
class Order extends Model {
    ...
    public function addItems(Collection $items) {
        ...
        foreach ($items as $item) {
            /** @var CartItem $item */
            $orderItems [] = OrderItem::create(['order_id' => $this->id, 'product_item_id' => $item->id, 'price' => ($item->qty * $item->price), 'quantity' => $item->qty]);
        }
    ...
    public function amountForTransfer() {
        return number_format($this->price + (substr($this->created_at, -2)*0.01), 2);
    }
